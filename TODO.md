...
## ✅ v2.9 (已完成 - Context 引用系統重構)

根據使用者體驗反饋，目前 `@` 自動補全、資料夾按鈕、檔案注入等功能分散，造成 UI 混亂。本版本將重構 Context 系統為統一核心模組。

### 核心架構重構 (#38)
- [x] 建立 `ContextManager` 核心模組
  - [x] 定義 `ContextReference` 介面 (type, path, content, token)
  - [x] 统一管理 File/Folder/Selection/Symbol Reference
  - [x] Token Budget 計算與限制
- [x] 重構 Prompt Builder
  - [x] 從解析 `@` 字串改為使用 `references` 陣列
  - [x] 支援結構化 Prompt 格式
- [x] 統一所有 Context 入口
  - [x] `@` Completion Provider
  - [x] QuickPick Reference Picker
  - [x] Drag & Drop
  - [x] Folder Picker (移除獨立的資料夾按鈕)

### UI/UX 優化 (#39)
- [x] 合併 Folder Button + `@` (只保留一個入口)
- [x] `@` 同時支援 File / Folder (使用 VSCode CompletionItemKind)
- [x] 實作 Context Bubble UI (類似 Cursor)
  - [x] 顯示檔案/資料夾名稱與統計資訊
  - [x] 不依賴解析 `@` 字串
- [x] 實作 QuickPick Reference Picker
  - [x] Fuzzy Search 支援
  - [x] 即時預覽功能

### Completion Provider 優化 (#40)
- [x] 移除自訂 `scanDirectories()` (改用 `workspace.findFiles()`)
- [x] 使用 `RelativePattern` 與 `FileSystemWatcher`
- [x] Directory 不 cache，從 file path 動態組裝
- [x] 減少 200 行 scanDirectories() 代碼
...
**Release**: v2.9.0 (2026-07-31)
**PR**: #42
**Issues**: #38, #39, #40, #41 (已關閉)
